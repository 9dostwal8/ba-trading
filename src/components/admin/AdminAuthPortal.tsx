...
import { lookupAdminLoginEmail } from "@/lib/admin-users.functions";
...
export function AdminAuthPortal({ onSuccess }: AdminAuthPortalProps) {
    ...
    try {
      ...
      if (!trimmed.includes("@")) {
        ...
        try {
          ...
          console.warn("Phone lookup fallback:", e);
        }

        // 3. Additional standard domain fallbacks
        candidates.push(
          `${cleanPhone}@dentalstore.app`,
          `${withZero}@dentalstore.app`,
          `${cleanPhone}@batrading.com`,
          `${withZero}@batrading.com`,
          `${cleanPhone}@batrading.iq`
        );
        candidates = Array.from(new Set(candidates));
      } else {
        candidates = [trimmed.toLowerCase()];
      }
      ...
      // If email candidates failed and input was a phone number, try native phone auth
      if (!loginData && !trimmed.includes("@")) {
        const rawDigits = trimmed.replace(/\D/g, "");
        const cleanPhone = rawDigits.replace(/^00964/, "").replace(/^964/, "").replace(/^0/, "");
        try {
          const { data, error } = await supabase.auth.signInWithPassword({
  ...
  return (
    <div className="min-h-screen w-full bg-white dark:bg-slate-950 flex flex-col lg:flex-row text-slate-900 dark:text-slate-100 font-sans overflow-hidden">
      {/* 3D Visual Hero Panel (Edge-to-edge full height on desktop) */}
      <div className="hidden lg:flex lg:w-5/12 xl:w-1/2 relative overflow-hidden bg-gradient-to-tr from-purple-950 via-indigo-900 to-violet-950 p-8 xl:p-14 flex-col justify-between text-white shadow-2xl">
        <img
          src="/login-hero-3d.png"
          alt="3D Hero"
          className="absolute inset-0 size-full object-cover object-center pointer-events-none transition-transform duration-700 hover:scale-105"
        />
        <div className="absolute inset-0 bg-gradient-to-t from-slate-950/80 via-transparent to-slate-950/30 pointer-events-none" />
        
        {/* Top Brand Header over Hero */}
        <div className="relative z-10 flex items-center gap-3">
          <div className="inline-flex size-12 items-center justify-center rounded-2xl bg-white/10 backdrop-blur-md border border-white/20 text-white shadow-lg">
            <ShieldCheck className="size-6 stroke-[2.2]" />
          </div>
          <div>
            <h2 className="text-base font-black tracking-wide">BA Trading</h2>
            <p className="text-xs text-white/70">Dental Store Administration Portal</p>
          </div>
        </div>
        ...
        <div className="relative z-10 max-w-md">
          <p className="text-sm font-semibold text-white/90 leading-relaxed">
            {lang === "ar"
              ? "منصة الإدارة المركزية المتقدمة لمتجر دنتال ستور. تحكم كامل بالمنتجات، الطلبات، وصلاحيات الفريق بأعلى معايير الأمان."
              : lang === "ku"
                ? "پلاتفۆرمی پێشکەوتووی بەڕێوەبردنی کۆگای پزیشکی ددان. کۆنترۆڵی تەواوی بەرهەمەکان، داواکارییەکان، و دەسەڵاتەکان بە بەرزترین ئاستی پارێزراوی."
                : "Advanced administration portal for dental commerce operations and role-based access management."}
          </p>
        </div>
      </div>

      {/* Form Section (Edge-to-edge full width with generous centered content) */}
      <div className="flex-1 flex flex-col justify-between p-6 sm:p-10 md:p-14 lg:p-16 xl:p-20 overflow-y-auto">
        <div className="max-w-md mx-auto w-full my-auto">
          ...
          <div className="mb-8 text-start">
            ...
            <p className="mt-2 text-sm font-medium text-slate-500 dark:text-slate-400">
              {lang === "ar"
                ? "قم بتسجيل الدخول برقم الهاتف أو البريد الإلكتروني"
                : lang === "ku"
                  ? "چوونەژوورەوە بە ژمارەی مۆبایل یان ئیمەیڵ بۆ بەردەوامبوون"
